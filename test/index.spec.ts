...
const sampleFLACFilePath = resolvePath(__dirname, './2L-125_stereo-44k-16b_04.flac')
...
describe('get-audio-duration', function () {
  ...
  context('When using a readable stream', function () {
    it('Should return proper duration', async function () {
      ...
      expect(duration)
        .to.be.a('number')
        .that.is.closeTo(expectedFLACAudioDuration, expectedAudioDurationThreshold)
    })
  ...
  context('When using a file path', function () {
    it('Should return proper duration', async function () {
      ...
      expect(duration)
        .to.be.a('number')
        .that.is.closeTo(expectedOGGAudioDuration, expectedAudioDurationThreshold)
    })
